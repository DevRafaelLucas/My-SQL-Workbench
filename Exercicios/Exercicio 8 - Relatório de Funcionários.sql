...
CREATE VIEW relatorioObraFuncionarios AS

## Tabela da mais fraca até a mais forte 
SELECT ps.nome AS nomeFuncionarios, 
clb.funcao, 
clb.salario, 
pjt.nomeProjeto, 
obr.dataInicio, 
ps.nomeFantasia FROM obra obr
INNER JOIN projeto pjt ON pjt.id_projeto = obr.id_projeto
INNER JOIN colaborador clb ON clb.id_colaborador = pjt.id_colaborador
INNER JOIN pessoa ps ON  ps.id_pessoa = clb.id_pessoa