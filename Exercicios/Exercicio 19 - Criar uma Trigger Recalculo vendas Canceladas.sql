...
DELIMITER //
CREATE TRIGGER atualiza_venda_canceladas
AFTER UPDATE 
ON itens_venda FOR EACH ROW
BEGIN
	IF(NEW.cancelado <> OLD.cancelado) THEN
    UPDATE venda SET valor_total = (SELECT SUM(valor_total) FROM
    itens_venda WHERE id_venda = NEW.id_venda AND cancelado = 'N')
    WHERE id_venda = NEW.id_venda;
    END IF;
END //

SELECT SUM(valor_total) FROM itens_venda WHERE id_venda = 3;
SELECT * FROM venda;
UPDATE itens_venda SET cancelado = 'N' WHERE id_itens_venda = 5;
